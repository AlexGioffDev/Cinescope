        ...
        <div className="w-full h-auto flex flex-col gap-y-4">
            ...
            <div className="p-4 flex items-center justify-between max-w-7xl mx-auto w-full -mt-10 z-10">
                <div className="space-y-1 min-w-0">
                    ...
                    <h1 className="font-black text-xl text-white tracking-tight uppercase">{series.name}</h1>
                    ...
                    <div className="flex flex-col gap-y-2 justify-center">
                        {series.in_production && (<p className="text-sm text-white font-black">In Production</p>)}
                        <p className="text-sm text-stone-500">Number of Seasons: <span className="text-white font-black">{series.number_of_seasons}</span></p>
                        <p className="text-sm text-stone-500">Number of Episodes: <span className="text-white font-black">{series.number_of_episodes}</span></p>
                    </div>
                </div>
                <img src={`https:image.tmdb.org/t/p/w500/${series.poster_path}`} alt="" className="w-80 h-120 object-cover hidden md:block rounded-lg border border-white/30" />
            </div>
                ...
                <section className="space-y-2">
                    ...
                    <div className="w-full overflow-x-auto flex items-start gap-x-4 scrollbar-thin">
                        {series.recommendations
                            .map(c => (
                                <CardSection key={`movie-recommendations-${c.id}`} show={{ ...c, media_type: c.media_type, title: c.name, year: c.first_air_date }} />
                            ))}
                    </div>
                </section>
            </div>
